...
interface AppContextType {
  ...
  createDivision: (
    ...
    template?: 'kanban_3' | 'cluster_3' | 'weekday' | 'priority' | 'single'
  ) => Division;
  ...
  setSnapshotFrequency: (freq: 'daily' | '12h' | '6h' | 'manual') => void;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [theme, setThemeState] = useState<AppTheme>(() => {
    return (localStorage.getItem('wtm_theme') as AppTheme) || 'light';
  });
  ...
  const activePeriod = useMemo(() => {
    ...
    // Fallback virtual period so tasks can always be associated without crashing
    return {
      id: `per-default-${activeWorkspace.id}`,
      workspace_id: activeWorkspace.id,
      name: 'Giai đoạn chính',
  ...
  }, [activeWorkspace, activePeriodId, db.periods, currentUser?.id]);

  const activeDivision = useMemo(() => {
    if (!currentUser) return null;
    const wsId = activeWorkspace?.id;
    // Divisions for active workspace: either public, or owned by currentUser
    const allowedDivisions = db.divisions.filter(
      (d) =>
        (!wsId || d.workspace_id === wsId) &&
    ...
    if (activePeriod) {
      const matchPeriod = allowedDivisions.find((d) => d.period_id === activePeriod.id);
    ...
    if (activeWorkspace) {
      return {
        id: `div-default-${activeWorkspace.id}`,
        workspace_id: activeWorkspace.id,
        period_id: activePeriod?.id || `per-default-${activeWorkspace.id}`,
        name: 'Phân chia chung',
        description: 'Phân chia mặc định',
        visibility: 'public' as DivisionVisibility,
        layout_type: 'full' as DivisionLayoutType,
        color: '#4F46E5',
        icon: 'Layers',
        sort_order: 0,
        is_default: true,
  ...
  const activeBoardModeId = useMemo(() => {
    ...
    if (divId && divisionBoardModes[divId]) {
      return divisionBoardModes[divId];
    }
    return 'kanban';
  }, [activeDivision?.id, divisionBoardModes]);
  ...
  useEffect(() => {
    if (activeDivision && activeDivision.id !== previousDivisionIdRef.current) {
      ...
      if (activeDivision.layout_type) {
        setViewMode(activeDivision.layout_type);
      }
    }
  }, [activeDivision?.id]);
  ...
  const createDivision = (
    data: Omit<Division, 'id' | 'owner_id' | 'created_at' | 'updated_at'>,
    template: 'kanban_3' | 'cluster_3' | 'weekday' | 'priority' | 'single' = 'kanban_3'
  ) => {
    if (!currentUser) throw new Error('Yêu cầu đăng nhập');
    ...
    // 1. Initialize initial clusters based on selected template
    let initialClusters: Cluster[] = [];
    if (template === 'kanban_3') {
      initialClusters = [
        {
          id: `clu-${newDivId}-todo`,
          division_id: newDivId,
          name: 'Cần làm',
          description: 'Công việc mới tiếp nhận',
          color: '#6366F1',
          icon: 'ListTodo',
          sort_order: 1,
          is_collapsed: false,
    ...
    } else if (template === 'cluster_3') {
      initialClusters = [
        {
          id: `clu-${newDivId}-1`,
          division_id: newDivId,
          name: 'Cụm 1',
          description: 'Nhóm công việc 1',
          color: '#3B82F6',
          icon: 'Folder',
          sort_order: 1,
          is_collapsed: false,
      ...
      const days = [
        { id: 'wd-mon', name: 'Thứ 2', color: '#3B82F6' },
        { id: 'wd-tue', name: 'Thứ 3', color: '#6366F1' },
        { id: 'wd-wed', name: 'Thứ 4', color: '#8B5CF6' },
        { id: 'wd-thu', name: 'Thứ 5', color: '#EC4899' },
        { id: 'wd-fri', name: 'Thứ 6', color: '#F59E0B' },
        { id: 'wd-sat', name: 'Thứ 7', color: '#10B981' },
        { id: 'wd-sun', name: 'Chủ nhật', color: '#EF4444' },
      ];
      initialClusters = days.map((d, idx) => ({
        id: `clu-${newDivId}-${d.id}`,
        division_id: newDivId,
        name: d.name,
        description: `Kế hoạch ${d.name}`,
        color: d.color,
        icon: 'Calendar',
        sort_order: idx + 1,
        is_collapsed: false,
      ...
      const prios = [
        { id: 'pr-urgent', name: 'Khẩn cấp', color: '#EF4444' },
        { id: 'pr-high', name: 'Ưu tiên cao', color: '#F59E0B' },
        { id: 'pr-medium', name: 'Trung bình', color: '#3B82F6' },
        { id: 'pr-low', name: 'Thấp', color: '#64748B' },
      ];
      initialClusters = prios.map((p, idx) => ({
        id: `clu-${newDivId}-${p.id}`,
        division_id: newDivId,
        name: p.name,
        description: `Mức ưu tiên ${p.name}`,
        color: p.color,
        icon: 'Sparkles',
        sort_order: idx + 1,
        is_collapsed: false,
      ...
      initialClusters = [
        {
          id: `clu-${newDivId}-default`,
          division_id: newDivId,
          name: 'Cụm 1',
          description: 'Cụm mặc định',
          color: '#6366F1',
          icon: 'Folder',
          sort_order: 1,
          is_collapsed: false,
    ...
    // 2. Initialize Division Board Modes
    const divisionBoardMode: BoardMode = {
      id: `bm-${newDivId}-kanban`,
      division_id: newDivId,
      name: 'Kanban Tiến trình',
      description: 'Chế độ bảng Kanban tiến độ công việc',
      icon: 'FolderKanban',
      is_preset: false,
      clusters: initialClusters.map((c, idx) => ({
        id: c.id,
        name: c.name,
        color: c.color,
        sort_order: idx + 1,
    ...
    setDb((prev) => {
      const updated = {
        ...
        clusters: [...prev.clusters, ...initialClusters],
        board_modes: [...(prev.board_modes || []), divisionBoardMode],
      };
      saveDatabase(updated);
      saveToFirestore(updated).catch(() => {});
      return updated;
    ...
    setDivisionBoardModes((prev) => ({
      ...prev,
      [newDivId]: template === 'weekday' ? 'weekday' : template === 'priority' ? 'priority' : 'kanban',
    }));

    setActiveDivisionId(newDiv.id);
    if (initialClusters[0]) {
      setActiveClusterId(initialClusters[0].id);
    }
    logActivity(newDiv.workspace_id, 'Tạo Division', 'division', newDiv.id, {
      name: newDiv.name,
      visibility: newDiv.visibility,
      clusters_count: initialClusters.length,
    });
    return newDiv;
  };
  ...
  const bulkMoveTasks = (taskIds: string[], targetClusterId: string, modeId?: string) => {
    ...
    const currentMode = modeId || activeBoardModeId || 'kanban';

    setDb((prev) => {
      const updatedTasks = prev.tasks.map((t) => {
        if (!taskIds.includes(t.id)) return t;

        const updatedTask = { ...t, updated_at: new Date().toISOString() };

        if (currentMode === 'kanban') {
          updatedTask.cluster_id = targetClusterId;
          const targetCluster = prev.clusters.find((c) => c.id === targetClusterId);
          if (targetCluster) {
            updatedTask.division_id = targetCluster.division_id;
          }
  ...
  const createBoardMode = (name: string, description = '', clusters: BoardModeCluster[] = []) => {
    const newMode: BoardMode = {
      ...
      clusters: clusters.length > 0 ? clusters : [
        { id: `c-${Date.now()}-1`, name: 'Cột 1', color: '#6366F1', sort_order: 1 },
        { id: `c-${Date.now()}-2`, name: 'Cột 2', color: '#10B981', sort_order: 2 },
      ],
    };
  ...
  const createSnapshot = (
    ...
    if (divisionIdsToSave && divisionIdsToSave.length > 0) {
      // User explicitly selected divisions to save
      targetDivisions = workspaceDivisions.filter((d) => divisionIdsToSave.includes(d.id));
    } else if (autoGenerated) {
      // Auto snapshot mode:
      // Mặc định tích chọn tất cả division chung và division cá nhân của tài khoản,
      // loại ra bất kỳ division nào nằm trong danh sách bỏ chọn (auto_snapshot_excluded_division_ids)
      const excludedSet = new Set(db.settings?.auto_snapshot_excluded_division_ids || []);
      targetDivisions = workspaceDivisions.filter((d) => {
        if (excludedSet.has(d.id)) return false;
        // Public division
        if (d.visibility !== 'private') return true;
        // Private division -> only if current user is owner
        return d.owner_id === currentUser?.id;
      });
    } else {
      // Default manual without explicit array: all public + own private
      targetDivisions = workspaceDivisions.filter((d) => {
        if (d.visibility !== 'private') return true;
        return d.owner_id === currentUser?.id;
      });
  ...
  const rollbackSnapshot = (snapshotId: string) => {
    ...
    try {
      const restoredState = JSON.parse(target.data_state) as DatabaseState;
      restoredState.snapshots = db.snapshots;
      setDb(restoredState);
      saveDatabase(restoredState);
      saveToFirestore(restoredState);
      logActivity(activeWorkspaceId || undefined, 'Khôi phục Phiên bản', 'workspace', target.id, {
        snapshot_name: target.name,
      });
      return true;
    } catch (e) {
      console.error('Lỗi khi rollback snapshot:', e);
      return false;
    }
  };
  ...
  const rollbackDivisionsFromSnapshot = (
    ...
    if (!snap) return { success: false, message: 'Không tìm thấy bản sao lưu.' };

    if (snap.workspace_id && activeWorkspaceId && snap.workspace_id !== activeWorkspaceId) {
      return { success: false, message: 'Bản sao lưu này thuộc phòng làm việc khác, không thể khôi phục tại phòng này.' };
    }
    ...
    try {
      const parsedState = JSON.parse(snap.data_state) as DatabaseState;
      const targetDivs = (parsedState.divisions || []).filter((d) => divisionIds.includes(d.id));

      if (targetDivs.length === 0) {
        return { success: false, message: 'Các phân chia đã chọn không tồn tại trong bản sao lưu này.' };
      }

      // Check permissions: for private divisions, only owner can rollback
      for (const targetDiv of targetDivs) {
        if (targetDiv.visibility === 'private' && targetDiv.owner_id !== currentUser?.id) {
          return {
            success: false,
            message: `Phân chia "${targetDiv.name}" là Riêng tư, chỉ tài khoản sở hữu mới có quyền khôi phục.`,
          };
        }
        const existingDiv = db.divisions.find((d) => d.id === targetDiv.id);
        if (existingDiv && existingDiv.visibility === 'private' && existingDiv.owner_id !== currentUser?.id) {
          return {
            success: false,
            message: `Phân chia "${existingDiv.name}" hiện tại là Riêng tư, chỉ tài khoản sở hữu mới có quyền khôi phục.`,
          };
        }
      }

      // Prepare affected divisions for shared restore point
      const affected_divisions = targetDivs.map((div) => {
        const cCount = (db.clusters || []).filter((c) => c.division_id === div.id).length;
        const tCount = (db.tasks || []).filter((t) => t.division_id === div.id).length;
        return {
          id: div.id,
          name: div.name,
          visibility: div.visibility,
          clusters_count: cCount,
          tasks_count: tCount,
        };
      });

      // Auto create SHARED Restore Point so other users in room can see and revert if needed!
      const restorePoint: RestorePoint = {
        id: `rst-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
        workspace_id: activeWorkspaceId || null,
        name: `Trước khi khôi phục bởi ${currentUser?.display_name || 'Thành viên'}`,
        description: `Bản chụp toàn diện trước khi rollback ${targetDivs.length} phân chia: ${targetDivs.map((d) => d.name).join(', ')}`,
        created_by: currentUser?.id || 'unknown',
        created_by_name: currentUser?.display_name || 'Thành viên',
        created_at: new Date().toISOString(),
        affected_divisions,
      ...
      // Extract restored items
      const restoredClusters = (parsedState.clusters || []).filter((c) => validDivisionIdSet.has(c.division_id));
      const restoredTasks = (parsedState.tasks || []).filter((t) => validDivisionIdSet.has(t.division_id));
      const restoredBoardModes = (parsedState.board_modes || []).filter(
        (m) => m.division_id && validDivisionIdSet.has(m.division_id)
      );
      const restoredClusterIds = new Set(restoredClusters.map((c) => c.id));
      const restoredSmartAreaItems = (parsedState.smart_area_items || []).filter((item) =>
        restoredClusterIds.has(item.cluster_id)
      );

      setDb((prev) => {
        const otherDivisions = prev.divisions.filter((d) => !validDivisionIdSet.has(d.id));
        const otherClusters = prev.clusters.filter((c) => !validDivisionIdSet.has(c.division_id));
        const otherTasks = prev.tasks.filter((t) => !validDivisionIdSet.has(t.division_id));
        const otherBoardModes = (prev.board_modes || []).filter(
          (m) => !m.division_id || !validDivisionIdSet.has(m.division_id)
        );
        const otherSmartAreaItems = prev.smart_area_items.filter(
          (item) => !restoredClusterIds.has(item.cluster_id)
        );

        const updatedRestorePoints = [restorePoint, ...(prev.restore_points || [])].slice(0, 30);
      ...
      logActivity(activeWorkspaceId || undefined, 'Khôi phục Phân chia (Rollback)', 'workspace', snap.id, {
        snapshot_name: snap.name,
        divisions_count: targetDivs.length,
        division_names: targetDivs.map((d) => d.name).join(', '),
      });

      return {
        success: true,
        message: `Đã khôi phục thành công ${targetDivs.length} phân chia (${targetDivs.map((d) => d.name).join(', ')}). Điểm hoàn tác chung đã được tạo tự động để phòng trường hợp muốn hoàn tác lại!`,
      };
    } catch (err: any) {
      console.error('Lỗi khi khôi phục phân chia:', err);
      return { success: false, message: 'Lỗi xử lý dữ liệu khi khôi phục phân chia.' };
    }
  };
  ...
  const rollbackDivisionFromSnapshot = (
    ...
    return rollbackDivisionsFromSnapshot(snapshotId, [divisionId]);
  };

  const revertFromRestorePoint = (restorePointId: string): { success: boolean; message: string } => {
    try {
      const rp = (db.restore_points || []).find((r) => r.id === restorePointId);
      if (!rp) {
        return { success: false, message: 'Không tìm thấy điểm hoàn tác chung.' };
      }

      const revertedDb = JSON.parse(rp.data_state) as DatabaseState;

      // Create a safety restore point before reverting
      const safetyPoint: RestorePoint = {
        id: `rst-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
        workspace_id: activeWorkspaceId || null,
        name: `Trước khi hoàn tác về "${rp.name}"`,
        description: `Tự động tạo điểm bảo vệ trước khi hoàn tác`,
        created_by: currentUser?.id || 'unknown',
        created_by_name: currentUser?.display_name || 'Người dùng',
        created_at: new Date().toISOString(),
        affected_divisions: rp.affected_divisions,
        data_state: JSON.stringify(db),
      };

      setDb((prev) => {
        const allRestorePoints = [safetyPoint, ...(prev.restore_points || [])].slice(0, 30);
        const updated: DatabaseState = {
          ...revertedDb,
          restore_points: allRestorePoints,
          users: prev.users,
      ...
      logActivity(activeWorkspaceId || undefined, 'Hoàn tác về điểm khôi phục', 'workspace', rp.id, {
        restore_point_name: rp.name,
        original_author: rp.created_by_name,
      });

      return {
        success: true,
        message: `Đã hoàn tác dữ liệu thành công về trạng thái trước đó của ${rp.created_by_name}!`,
      };
    } catch (err: any) {
      console.error('Lỗi khi hoàn tác điểm restore:', err);
      return { success: false, message: 'Lỗi xử lý dữ liệu khi hoàn tác.' };
    }
  };
  ...
  const setSnapshotFrequency = (frequency: 'daily' | '12h' | '6h' | 'manual') => {
    setDb((prev) => {
      const updated = {
        ...prev,
        settings: {
